 ...
 *
 * 指示55-① で3つ目（'turn'）ができたので、**このゲームの締め切りを1つずつ見直した**。
 * 結論は `'play'` のまま——協力版も競争版も、**全員が同時にコードを切る**。手番で回らないので 3つ目（turn）には当たらない。
 *（見張りが `tests/room-paths.js` だという記述が長く残っていたが、
 *  room-paths に clockKind は1度も出てこない。実態は big-screen・落とし穴33）
 */
...
module.exports = {
  ...
  playersOf, expectedMembers, entryOf, resultView, clockKind
};
