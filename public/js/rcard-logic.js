...
(function (root, factory) {
   ...
   * **その試合で仕掛ける爆弾の数。**
   *
   * 「決勝の数」を使うのは、**生き残り戦で本当に残り2人まで来た時だけ**。
   * 最初から1対1（いっきうち・2人の部屋）は、**選んだ数をそのまま使う**。
   *
   * ※**ここは本人の指示で変えた（2026-09-21）。**
   *   それまでは生存者数だけを見ていたので、**いっきうちは必ず決勝あつかい**になり、
   *   設定の「しかける爆弾の数」を何に変えても **5つのまま**だった——
   *   **設定は出るのに、何も起きない**（落とし穴21）。
   *   以前の理由は「ここを生き残り戦だけにすると、2人で遊ぶ人は一生5つを見ない」。
   *   その心配は**設定で5つをえらべる**ようにして解いた——
   *   遊びの側で固定するのではなく、選べるようにする方で解く。
   *
   * @param 生存者数 いま、この試合に残っている人数
   * @param cfg      設定
   * @param 開始人数  **その回を始めた時の人数。**
   *                  省略したら「生存者数と同じ」＝まだ誰も減っていない＝
   *                  最初から1対1、と読む。**渡されないと「勝ち抜いて2人になった」
   *                  のかは分からないので、分からない時は決勝あつかいにしない。**
   *                  （うっかり省略した時に、直したはずの5つが復活しないように）
   */
  function bombsForMatch(生存者数, cfg, 開始人数) {
    var c = normalizeConfig(cfg);
    var 始め = (開始人数 == null) ? 生存者数 : 開始人数;
    if (始め <= 2) return c.bombs;                 // 最初から1対1
