 ...
 *
 * ---- ただし、締め切りには3種類ある（指示55-① で2種類から増やした） ----
 * | 種類 | 誰が待っているか | 例 | 帯の時計 | 巨大カウントダウン |
 * |---|---|---|---|---|
 * | `'play'` | **卓のみんな** | 話し合いの残り・解除の持ち時間・入札の締切 | 出す | **出す** |
 * | `'turn'` | **手番の人だけ** | ロシアンカードの「めくるまでの15秒」 | 出す | **出さない** |
 * | `'tick'` | 誰も待っていない（画面が変わるだけ） | とくとくクイズの「次の1文字が出る時刻」 | 出さない | 出さない |
 *
 * **なぜ `'turn'` を足したか**（指示55-①・本人の裁定 2026-09-19）。
 * `.big-cd` は残り5秒から1秒まで**毎秒1枚ずつ** 420px の数字を全面に出す。
 * 手番ごとに回る15秒の締め切りだと、**1手番の3分の1の時間、卓の真ん中
 * （3×3の盤を置く場所）を数字が覆い続ける**——§11-4 が
 * 「状況でないものが主役を隠している」として名指しで直した形が、15秒周期で再発する。
 *
 * かといって時計そのものを消すと、手番の人が「あと何秒か」を知る手がないし、
 * `w.deadline` は**時間切れの自動進行と兼用**なので、置かない選択は
 * 「15秒で勝手に進む」を自前で書き直すことになる。
 * だから**帯の時計は出し、巨大カウントダウンだけ出さない**——
 * 正本 §11-4 の「どちらかは進行役が言う」を、そのまま3つ目へ延ばした形。
 *
 * 区別しないと、**420px の数字が伏せ字の問題文の上に1〜2秒ごとに出続ける**
 * （実測：6秒間に6回）。どれかは**進行役が言う**（`clockKind(room)`）——
 * 端末が締め切りの中身を当てにいくと、締め切りの意味を変えた日に遠くで壊れる。
 * 全進行役が名乗っていることは `tests/big-screen.js` が両方向で見張る
 * （**このコメントは長く「tests/room-paths.js が見張る」と書いていたが、
 *   room-paths に clockKind は1度も出てこない**。指示55-① で実態に直した・落とし穴33）。
 */
const CLOCK_KINDS = ['play', 'turn', 'tick'];
function 時計をそえる(room, view) {
  ...
  // **不在で表さない**（落とし穴36）。知らない種類は「出さない」に倒す——
  // 名乗り間違いで巨大な数字が出続けるより、出ない方が安全側
  const 数える = (kind === 'play' || kind === 'turn');
  view.clock = (w && w.deadline && 数える)
    ? { ms: Math.max(0, w.deadline - Date.now()), kind: kind }
    : null;
  return view;
}
