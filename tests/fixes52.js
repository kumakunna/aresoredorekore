...
(async () => {
  ...
  // だからここはCSSの**錨**を見る——別々の角に錨を下ろしていること。
  // 実際のかぶりは実ブラウザで測って、その数字を上に残す（落とし穴12）。
  //
  // **第57弾：3つ目の印（「ここをタップ」）はもう無い。**
  // 初プレイの案内はチュートリアル（tutorial.js）が引き受けたので、
  // 案内の位置と「✕ の付くマスには出ない」の2件をここから外した。
  // 残る2つ（番号・✕）の錨は、そのまま見張り続ける

  await r.test('52-5：番号は右下、✕ は左上——2つが別々の角にいる', async () => {
    const html = fs.readFileSync(path.join(__dirname, '..', 'public', 'index.html'), 'utf8');
    const css = html.slice(html.indexOf('<style>') + 7, html.indexOf('</style>'))
      .replace(/\/\*[\s\S]*?\*\//g, '');
    const 規則 = cssRules(css);
  ...
  // ===================== 52-6 爆発は「演出 → 結果」の順 =====================
  //
  // 本人の実機フィードバック：**演出が終わってから**結果の文字を出す。
  //
  ...
  //
  // 第59弾：爆発の中身は `FxKit.burst('💥')`（拡大縮小）→ `FxKit.shutter(...)`
  //（落ちてくる帯）に差し替わったが、**「演出が終わってから文字」という順そのものは同じ**。
  // `node tools/fx-probe.js bomb coop`（本物の進行役を動かす道具）でも確かめた：
  //
  //   ミス3    爆発:1 帯:0   ← 💥が出て、文字はまだ無い
  //   あと     爆発:1 帯:0
  //   少しあと  爆発:0 帯:1   ← 💥が消えてから、文字が出た
  //
