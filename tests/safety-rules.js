...
(async function main() {
  ...
  await r.test('赤は縁のみ。塗ってよいのは主ボタン1つだけ（正本6）', async () => {
    // 赤い面は、それだけで「叱られている」に見える。
    // 原則C（責める時は静かに）と同じ理由で、**地**は輪郭にとどめる。
    //
    // ただし主ボタンだけは塗る。ここまで地味にすると
    // **元にもどせない操作が、ふつうの操作と見分けられなくなる**——
    // 安全のための決まりが、安全を減らす（第39弾・実ブラウザで見て決めた）
    const 危険 = CSS.match(/\.ui-dialog-in\.is-danger\{[^}]*\}/);
    assert(危険, '取り返しがつかないダイアログの指定がある');
    assert(/border/.test(危険[0]), '赤は縁で出す');
    assert(!/background:\s*var\(--stamp\)/.test(危険[0]), '地は赤で塗らない');

    // 塗ってよいのは1つまで。**数を数える**——
    // 「縁のみ」とだけ書いて数を見ないと、逃げ道まで赤くなった日に気づけない
    const 赤く塗る = (CSS.match(/\.is-danger[^{]*\{[^}]*background:\s*var\(--stamp[^)]*\)/g) || []);
    assert(赤く塗る.length <= 1,
      '赤で塗るのはダイアログにつき1つまで（実際:' + 赤く塗る.length + '件）');
  });
