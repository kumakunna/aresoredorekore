...
async function run() {
  ...
  await r.test('57-5：幕は指を受け、光の点滅を切ると輪が止まる', async () => {
    ...
    // **押す的は 44px を割らない**（正本§7-0e「押す的（44px）は1つも縮めない」）。
    // この下限はアプリ全体の門になっていないので、機械は何も言わない——
    // 実際、指示57 は 40px で書いて仕上げまで気づかなかった。
    // 全体の門にするのは別の話なので、**ここで作った的だけ**は数える
    const 閉じる = 規則('.tut-x');
    ['width', 'height'].forEach((k) => {
      const m = 閉じる.match(new RegExp('(^|;|\\s)' + k + '\\s*:\\s*([0-9.]+)px'));
      assert(m, '.tut-x に ' + k + ' がある');   // 型(b)：読めていないまま緑にしない
      assert(parseFloat(m[2]) >= 44,
        '閉じるの ' + k + ' が 44px を割っている（' + m[2] + 'px・正本§7-0e）');
    });
  });
