...
const SITES = [
  ...
  { 何: '部屋をとじる', 探す: '部屋をとじますか？', 種類: 'danger' },
  ...
  { 何: '部屋がとじられた', 探す: '進行役が部屋をとじました', 種類: 'info' },
  // 第41弾 2-15：進行役をやめる。部屋ごと閉じるので、取り返しがつかない側
  { 何: '自分が参加者になる', 探す: '自分が参加者になりますか？', 種類: 'danger' },
...
(async function main() {
  ...
  await r.test('動詞はかな、固有の名詞は漢字のまま（第40弾 40-Cの裁定）', async () => {
    ...
    // **お題データ（QUIZ_BANK）は外す。**「ファスナー」の封印語に「閉める」があるが、
    // それは遊ぶ人が言ってはいけない語であって、アプリの言葉づかいではない。
    // ほかのトーンの検査はすでに 校正の対象() を見ていて、**ここだけ 画面の文言() のままだった**
    const 全部 = 校正の対象();
    assert(全部.length > 1800, '画面の文言を数えられている（実際:' + 全部.length + '件）');  // 型(b)

    // 「開き閉て」「開け閉て」は**建具の決まった言い方**（オークションの品物の説明）。
    // 『閉じる→とじる』のような動詞の言い換えとは種類が違うので、先に伏せる
    const 固有 = ['手渡し', '決選投票', '開き閉て', '開け閉て'];
    const 見つけた = [];
    全部.forEach((x) => {
      // 固有の名詞を伏せてから、動詞の漢字が残っていないかを見る
      let t = x.t;
      固有.forEach((w) => { t = t.split(w).join(''); });
      // 指示49：**`閉` を足した。**「部屋を閉じますか？」の見出しと「とじる」の札が、
      // **同じダイアログの中で割れていた**（本人の指摘）。
      // BANNED に 閉じる→とじる と書いてあったのに残っていたのは、
      // 見張りが**ボタンの札だけ**を掃いていたうえ、
      // strip() の穴で**その札7つすら見えていなかった**から（tools/ui-text-scan.js 参照）。
      // ここに入れると、本文もサーバーも ui-text.js も、まとめて掃かれる
      const 残り = t.match(/[渡戻]|[選閉](?=[ぁ-ん])/g);
      if (残り) {
